...
A CA can prevent Man-in-the-Middle attacks. A malicious individual can try to intercept connections by attempting to act as a server and binding their own key to the server identity. If digital certificates were not provided, then the client has no way of knowing that the server attemtping to form a connection is not in fact the server that it claims to be. However, with certificates, the fake public key will be detected and the malicious server, or "man in the middle", will not be incorrectly verified. 
